...
find valid path/s to the exit. The text file that is read in contains characters E(Entrance), X(Exit), W(wall), 
and P(path). There is only one E and it will be found on the 0 column of the maze. There is only one 'X' and it can be 
found on any of the other 3 walls besides the 0 column. All other boarders will be 'W'. The only valid place that the 
agent can take is the 'P' locations which are randomly spread out throughout the map. Some paths may lead to a dead end,
...
A map is then created with the key value pair of the list index(maze location) as the key, and a list containing 
the location, data, and visited bool as the value for each key. The program will then display the maze to the console
